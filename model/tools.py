...
def make_layers(kernels, func, zscore_module={}):
    # kernels:dict
    # Convolution kernels made by memes.get_kernels()
    ...
    for key in kernels:
        ...
        if func == memes_ED_mask:
            layers[key] = func(meme_kernel, meme_channel)
        elif func == zscore_ED_mask:
            layers[key] = func(meme_kernel, meme_channel, zscore_module[where])
        elif func == zscore_sim_mask:
            threshold = memes['thresholds']
            std = Standardization(torch.Tensor(threshold))
            layers[key] = func(meme_kernel, meme_channel, zscore_module[where], std)
        else:
            layers[key] = func(meme_kernel)
...
def get_parameters(model,classifier_keywords=["fc","classifier"]):
    #Basic tools for classifying backbones and classifiers
    #keywords:list
    ...
    for name,param in model.named_parameters():
        for keyword in classifier_keywords:
            if keyword in name:
                ...
                break
    base_params = list(filter(lambda param: id(param) not in
                        list(map(id,classifiers_params)),model.parameters()))
    return base_params,classifiers_params

def get_remains(model,param_list):
    #Get unnecessary parameters
    ...
    for name,layer in layers.items():
        for keyword in keywords:
            if keyword in name:
                ...
                break
        if break2:break
        optional_layers.append(name)
        avaliable_layers.append(layer)
    return optional_layers, avaliable_layers
...
def data_generator(all_data, batch_size):
    while True:
        output = [data[:batch_size] for data in all_data]
        all_data = [data[batch_size:] for data in all_data]
        yield output

def compute_min_distance(kernels,channel,feature_map,
            use_gpu = True, batch_size = 32):
    if kernels.ndim == 3:
        kernels = expand_memes(kernels, feature_map.shape[1], channel)
    ...
    return (torch.min(torch.min(dist, 2).values, 2).values).detach().numpy()
...
def crop_bbox(dist_map, thres, times = 1.05):
...
def expand_memes(kernels, size, channel):
    amount, length, width = kernels.shape
    mask = np.zeros((amount, size, length, width), dtype=np.float32)
    mask[range(amount), channel] = kernels
    return mask
...
class Standardization(nn.Module):
    ...
    def __call__(self, data):
        #output = 1 - 1 / (1 + np.exp(self.threshold - data))
        if self.mode:
